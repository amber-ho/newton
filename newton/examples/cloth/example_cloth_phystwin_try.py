...
from newton.solvers import SolverFeatherstone, SolverSemiImplicit
...
class Example:
    def __init__(self, viewer, args):
        ...
        # add PhysTwin cloth as mass-spring particles
        if self.add_cloth:
            self._add_phystwin_cloth_to_scene()
            self.scene.color()

        self.scene.add_ground_plane()

        self.model = self.scene.finalize(requires_grad=False)
        if self.add_cloth and self.spring_count > 0:
            self.model.spring_rest_length.assign(self.phystwin_rest_lengths_cm)
        ...
        self.cloth_solver: SolverSemiImplicit | None = None
        if self.add_cloth:
            self.cloth_solver = SolverSemiImplicit(self.model)
    ...
    def _load_phystwin_export(self) -> None:
        with np.load(self.npz_path) as data:
            self.object_positions_m = np.array(data["object_vertices_0"], dtype=np.float32)
            self.object_masses = np.array(data["masses"], dtype=np.float32)
            self.spring_indices = np.array(data["springs"], dtype=np.int32)
            self.spring_rest_lengths_m = np.array(data["rest_lengths"], dtype=np.float32)
            ...
            self.phystwin_particle_radius_m = float(data["collision_dist"]) if "collision_dist" in data else 0.02
            self.contact_friction = float(data["collide_fric"]) if "collide_fric" in data else self.self_contact_friction
            self.contact_restitution = float(data["collide_elas"]) if "collide_elas" in data else 0.0

        if self.object_positions_m.ndim != 2 or self.object_positions_m.shape[1] != 3:
            raise ValueError("`object_vertices_0` must have shape (N, 3).")

        if self.object_masses.shape[0] != self.object_positions_m.shape[0]:
        ...
        if self.spring_rest_lengths_m.shape[0] != self.spring_indices.shape[0]:
        ...
        self.object_particle_count = self.object_positions_m.shape[0]
        self.spring_count = self.spring_indices.shape[0]

        if self.spring_count > 0 and int(self.spring_indices.max()) >= self.object_particle_count:
            raise ValueError("`springs` references particle index outside object_vertices_0.")

        self.spring_stiffness, self.spring_damping, self.resolved_stiffness_mode = self._decode_phystwin_springs()


    def _decode_phystwin_springs(self) -> tuple[np.ndarray, np.ndarray, str]:
        ...
        return spring_stiffness.astype(np.float32), spring_damping.astype(np.float32), mode


    def _add_phystwin_cloth_to_scene(self) -> None:
        # cloth_franka uses centimeter scale.
        # PhysTwin export is assumed to be meters, so multiply by 100.
        positions_cm = self.object_positions_m * 100.0

        # Move the PhysTwin object onto the table.
        # Table top z = table_pos_z + table_hz = 10 + 10 = 20 cm.
        p_min = positions_cm.min(axis=0)
        p_max = positions_cm.max(axis=0)
        center = 0.5 * (p_min + p_max)

        target_center_xy = np.array([0.0, -50.0], dtype=np.float32)
        table_top_z = float(self.table_pos_cm[2] + self.table_hz_cm)

        offset = np.array(
            [
                target_center_xy[0] - center[0],
                target_center_xy[1] - center[1],
                table_top_z + self.cloth_particle_radius - p_min[2],
            ],
            dtype=np.float32,
        )

        positions_cm = positions_cm + offset

        # PhysTwin collision_dist is in meters, convert to cm.
        self.cloth_particle_radius = max(
            0.2,
            float(self.phystwin_particle_radius_m * 100.0),
        )

        for pos, mass in zip(positions_cm, self.object_masses):
        ...
        # rest length also needs meter -> cm conversion
        self.phystwin_rest_lengths_cm = self.spring_rest_lengths_m * 100.0
    ...
    def create_articulation(self, builder):
        ...
        self.robot_key_poses = np.array(
            ...
            # ],
            [
                # duration, x, y, z, qw, qx, qy, qz, gripper
                [2.0,  0.0, -20.0, 30.0, 1, 0.0, 0.0, 0.0, clamp_open_activation_val],   # 到衣服上方
                [2.0,  0.0, -20.0, 20.0, 1, 0.0, 0.0, 0.0, clamp_open_activation_val],   # 往下
                [1.0,  0.0, -20.0, 20.0, 1, 0.0, 0.0, 0.0, clamp_close_activation_val],  # 夾住
                [2.0,  0.0, -20.0, 35.0, 1, 0.0, 0.0, 0.0, clamp_close_activation_val],  # 往上提
                [1.5,  0.0, -20.0, 35.0, 1, 0.0, 0.0, 0.0, clamp_close_activation_val],  # 停一下
                [1.0,  0.0, -20.0, 35.0, 1, 0.0, 0.0, 0.0, clamp_open_activation_val],   # 放開（可刪）
            ],
            dtype=np.float32,
        )
